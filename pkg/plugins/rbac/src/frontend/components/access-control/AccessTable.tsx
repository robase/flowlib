      ...
      <DropdownMenuTrigger asChild>
        <button
          ...
          className={cn(
            ...
            value
              ? getPermissionBadgeClasses(value)
              : 'border-fl-border text-fl-muted-foreground hover:bg-fl-muted/50',
          )}
        >
...
export function AccessTable({
  ...
  if (isLoading) {
    return <div className="px-2 py-4 text-sm text-center text-fl-muted-foreground">Loading…</div>;
  }
  if (rows.length === 0) {
    return (
      <div className="px-2 py-4 text-sm text-center text-fl-muted-foreground">{emptyLabel}</div>
    );
  }
    ...
    <>
            ...
            <Fragment key={group.label || '_default'}>
              {hasGroups && group.label && (
                <tr>
                  <td
                    colSpan={3}
                    className="px-3 pt-4 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-fl-muted-foreground"
                  >
                    {group.label}
                  </td>
                </tr>
              ...
              {group.rows.map((row, rowIndex) => (
                <tr
                  key={row.id}
                  className={cn(
                    'hover:bg-fl-muted/20',
                    rowIndex > 0 && 'border-t border-fl-border',
                  )}
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={cn(
                          ...
                          row.kind === 'user' ? 'bg-fl-primary/10' : 'bg-fl-muted',
                        )}
                      >
                        {row.kind === 'user' ? (
                          <User className="h-3.5 w-3.5 text-fl-primary" />
                        ) : (
                          <Users className="h-3.5 w-3.5 text-fl-muted-foreground" />
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {row.kind === 'team' && (
                            <span className="shrink-0 rounded border border-fl-border px-1.5 py-0.5 text-[11px] font-medium text-fl-muted-foreground">
                              Team
                            </span>
                          )}
                        </div>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type="button"
                              className={cn(
                                'inline-flex w-32 items-center justify-between gap-1 rounded-full border px-2.5 py-1 text-xs font-medium capitalize hover:bg-fl-muted/50',
                  ...
                  <td className="px-3 py-2.5 text-center">
                    {row.canRemove && row.onRemove ? (
                      <button
                        ...
                        className="p-1 rounded text-fl-muted-foreground hover:bg-fl-destructive/10 hover:text-fl-destructive"
                      >
      ...
      >
        <DialogContent className="max-w-sm border-fl-border bg-fl-background text-fl-foreground sm:max-w-sm">
          ...
          <p className="text-sm text-fl-muted-foreground">{removalDialogCopy?.body}</p>
          <DialogFooter className="justify-between w-full sm:gap-2">
            <button
              type="button"
              onClick={() => setPendingRemovalRow(null)}
              className="rounded-md mr-auto border border-fl-border px-3 py-1.5 text-xs font-medium text-fl-muted-foreground hover:text-fl-foreground"
            >
              Cancel
            </button>
            <button
