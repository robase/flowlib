...
export class RolePermissionsRepository {
  ...
  async list(filter: ListRolePermissionsFilter = {}): Promise<AgentRolePermission[]> {
    let query = this.database.kysely<AgentsDB>().selectFrom('agent_role_permissions').selectAll();
  ...
  async create(input: UpsertRolePermissionInput): Promise<AgentRolePermission> {
    ...
    if (!created) {
      throw new Error('Failed to load created role permission');
    }
    return created;
  }
  ...
  async update(
    ...
    if (patch.enabled !== undefined) {
      set.enabled = boolFor(this.database, patch.enabled);
    }
  ...
  async upsert(input: UpsertRolePermissionInput): Promise<AgentRolePermission> {
    ...
    if (existing) {
      ...
      if (!updated) {
        throw new Error('Failed to upsert role permission');
      }
      return updated;
    }
