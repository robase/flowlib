  ...
  return (
    ...
    >
      ...
      <div className="px-6 py-6">
        {isLoading ? (
          <div className="text-sm text-fl-muted-foreground">Loading…</div>
        ) : error ? (
          <ErrorState message={(error as Error).message} />
        ) : !servers || servers.length === 0 ? (
          <div className="rounded-md border border-dashed border-fl-border px-6 py-12 text-center text-sm text-fl-muted-foreground">
  ...
  return (
    <li className="flex items-center justify-between gap-4 px-2 py-3">
      ...
      <button
        ...
        onClick={onDelete}
        disabled={deleting}
        className="text-xs text-fl-destructive hover:underline disabled:opacity-50"
      >
        Remove
      </button>
    </li>
...
function CreateForm({
  ...
  return (
    <form
      ...
      className="border-b border-fl-border bg-fl-card/40 px-6 py-4 space-y-3"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <label className="block">
          <span className="text-xs font-medium text-fl-muted-foreground">Name</span>
          <input
            type="text"
            required
            value={name}
        ...
        <label className="block">
          <span className="text-xs font-medium text-fl-muted-foreground">Transport</span>
          <select
            value={transport}
      ...
      <label className="block">
        <span className="text-xs font-medium text-fl-muted-foreground">Description (optional)</span>
        <input
          type="text"
          value={description}
      ...
      <label className="block">
        <span className="text-xs font-medium text-fl-muted-foreground">Config (JSON)</span>
        <textarea
          value={configText}
        ...
        />
        {parseError && <div className="text-xs text-fl-destructive mt-1">{parseError}</div>}
      </label>
      <div className="flex gap-2">
        <button
          type="submit"
          disabled={submitting || !name.trim()}
          className="rounded-md bg-fl-primary px-4 py-1.5 text-sm font-medium text-fl-primary-foreground hover:opacity-90 disabled:opacity-50"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="rounded-md border border-fl-border px-4 py-1.5 text-sm hover:bg-fl-card"
        >
          Cancel
        </button>
      </div>
