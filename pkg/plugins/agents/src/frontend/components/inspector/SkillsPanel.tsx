        ...
        {isLoading ? (
          ...
          skills.map((skill) => (
            <button
              ...
              className="block w-full rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-fl-muted/40"
            >
              <div className="flex items-center gap-2">
                <span
                  className={cn(
                    'h-1.5 w-1.5 shrink-0 rounded-full',
                    skill.scope === 'global' ? 'bg-fl-success' : 'bg-fl-muted-foreground/40',
                  )}
                  aria-label={skill.scope === 'global' ? 'Org-wide' : 'Personal'}
                />
                <code className="truncate font-mono text-sm font-medium text-fl-foreground">
                  {skill.name}
                </code>
              </div>
              <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-fl-muted-foreground">
                {skill.description}
              </p>
            </button>
          ))
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 px-3 pt-2 pb-2">
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md text-fl-muted-foreground hover:bg-fl-muted/40 hover:text-fl-foreground"
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <BookOpen className="h-4 w-4 shrink-0 text-fl-muted-foreground" />
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 pt-2">
        <p className="text-sm leading-relaxed text-fl-muted-foreground">{skill.description}</p>
        <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-fl-muted-foreground">
        ...
        <pre className="mt-4 overflow-x-auto whitespace-pre-wrap rounded-lg bg-fl-muted/40 p-3 font-mono text-xs leading-relaxed text-fl-foreground">
          {skill.body}
        </pre>
      </div>
    </div>
