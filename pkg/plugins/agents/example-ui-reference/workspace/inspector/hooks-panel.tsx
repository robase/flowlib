...
const EVENT_META: Record<AuditEventType, { icon: typeof ShieldAlert; className: string; label: string }> = {
        ...
        {hooks.map((hook) => (
          <div key={hook.id} className="rounded-md px-3 py-2 hover:bg-muted/40">
            <div className="flex items-center gap-2">
              <code className="truncate font-mono text-xs font-medium text-foreground">{hook.name}</code>
              <span className="shrink-0 text-[11px] text-muted-foreground">{hook.phase}</span>
              <Switch className="ml-auto scale-90" checked={hook.enabled} aria-label={`Toggle ${hook.name}`} />
            </div>
              ...
              <li key={event.id} className="flex gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <code className="truncate font-mono text-[11px] text-muted-foreground">{event.tool}</code>
                    <span className="ml-auto shrink-0 text-[11px] text-muted-foreground">{relativeTime(event.createdAt)}</span>
                  </div>
                  <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">{event.reason}</p>
                </div>
              </li>
            )
          })}
