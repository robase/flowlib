      ...
      <div className="px-4 pt-2 pb-3">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
          ...
          <PolicyStat
            label="Allow-list"
            value={
              policy.enabledTools.length === 0
                ? 'all allowed'
                : `${policy.enabledTools.length} tools`
            }
          />
          <PolicyStat
            label="Output budget"
            value={`${policy.toolOutputBudget.lines} ln / ${policy.toolOutputBudget.bytes} B`}
          />
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {tool.state !== 'enabled' && (
                    <span className={cn('ml-auto shrink-0 text-[11px]', meta.className)}>
                      {meta.label}
                    </span>
                  )}
                </div>
