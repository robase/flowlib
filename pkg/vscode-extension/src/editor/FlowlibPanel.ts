/**
 * Opens a non-file-backed Flowlib webview tab — credentials, webhooks,
...
export class FlowlibPanelProvider implements vscode.Disposable {
  ...
  /**
   * Open (or focus) a panel showing the Flowlib UI deep-linked to
  ...
  private async sendInit(panel: vscode.WebviewPanel, initialPath: string): Promise<void> {
    try {
      ...
      getExtensionLogger().error('FlowlibPanel sendInit failed', {
