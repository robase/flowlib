/**
 * Drizzle Schema Generators
 *
 * Converts the merged abstract schema (core + plugins) into three
 * dialect-specific Drizzle ORM schema files as TypeScript source code.
 *
 * Each generator produces a complete, self-contained .ts file that can
 * be used by Drizzle Kit for migration generation.
 */

import type { PluginFieldAttribute, PluginFieldType } from './plugin-schema';
import type { MergedSchema, MergedTable } from './schema-merger';

// =============================================================================
// Common Utilities
// =============================================================================

/**
 * Convert camelCase to snake_case for DB column names.
 */
function toSnakeCase(str: string): string {
  return str.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
}

/**
 * Check if a field type is an enum (string array).
 */
function isEnumType(type: PluginFieldType): type is string[] {
  return Array.isArray(type);
}

/**
 * Generate a variable name for an enum from a table + field.
 * e.g., flowRuns + status → flowRunStatusEnum
 */
function enumVarName(tableName: string, fieldName: string): string {
  return `${tableName}${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)}Enum`;
}

// Path A (`plans/db-package-split-plan.md`): emit imports from
// `@flowlib/action-kit` (types-only, < 50 KB gzip) instead of `@flowlib/core`
// (the executor, ~18 MB raw). Generated `flowlib.schema.ts` files in
// consumer repos become Worker-safe by default.
const SQLITE_RUNTIME_DEFAULT_IMPORTS: Record<string, string> = {
  BatchProvider: '@flowlib/action-kit',
  BatchStatus: '@flowlib/action-kit',
  FlowRunStatus: '@flowlib/action-kit',
  NodeExecutionStatus: '@flowlib/action-kit',
};

function collectSqliteRuntimeDefaultImports(schema: MergedSchema): Map<string, Set<string>> {
  const importsByModule = new Map<string, Set<string>>();

  for (const table of schema.tables) {
    for (const field of Object.values(table.definition.fields)) {
      if (typeof field.defaultValue !== 'string' || !field.typeAnnotation) {
        continue;
      }

      const importPath = SQLITE_RUNTIME_DEFAULT_IMPORTS[field.typeAnnotation];
      if (!importPath) {
        continue;
      }

      const existing = importsByModule.get(importPath) ?? new Set<string>();
      existing.add(field.typeAnnotation);
      importsByModule.set(importPath, existing);
    }
  }

  return importsByModule;
}

// =============================================================================
// SQLite Generator
// =============================================================================

export function generateSqliteSchema(schema: MergedSchema): string {
  const lines: string[] = [];
  const runtimeImports = collectSqliteRuntimeDefaultImports(schema);
  const hasIndexes = schemaHasInjectedIndexes(schema);

  // Header
  lines.push(`// SQLite schema for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit manually. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(``);
  lines.push(
    `import { sqliteTable, text, integer, ${hasIndexes ? 'index, ' : ''}primaryKey, type AnySQLiteColumn } from 'drizzle-orm/sqlite-core';`,
  );
  lines.push(`import { relations, sql } from 'drizzle-orm';`);
  // Emit as `import type` so the consumer's bundler (wrangler/esbuild) doesn't
  // need to resolve `@flowlib/action-kit` at runtime — generated schema files
  // commonly land in repos where this package isn't hoisted to the workspace
  // root. The values themselves are inlined as string literals at the
  // `.default(…)` call sites (see `sqliteDefault`).
  for (const [importPath, identifiers] of runtimeImports.entries()) {
    lines.push(
      `import type { ${Array.from(identifiers).sort().join(', ')} } from '${importPath}';`,
    );
  }
  lines.push(`import { randomUUID } from 'crypto';`);
  lines.push(``);

  // Collect names already imported as runtime values to avoid duplicates in type import
  const runtimeImportedNames = new Set<string>();
  for (const identifiers of runtimeImports.values()) {
    for (const id of identifiers) {
      runtimeImportedNames.add(id);
    }
  }

  // Type imports from @flowlib/core/types
  const typeImport = collectTypeImportsForAppend(schema, runtimeImportedNames);
  if (typeImport) {
    lines.push(typeImport);
  }

  // Local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema, runtimeImportedNames);
  if (localAliases.length > 0) {
    lines.push(``);
    for (const alias of localAliases) {
      lines.push(alias);
    }
  }
  lines.push(``);

  // Tables
  lines.push(`// =============================================================================`);
  lines.push(`// Tables`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  for (const table of schema.tables) {
    lines.push(generateSqliteTable(table, schema));
    lines.push(``);
  }

  // Relations
  lines.push(`// =============================================================================`);
  lines.push(`// Relations`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  lines.push(generateRelations(schema));

  // Type exports
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Type exports`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  for (const table of schema.tables) {
    const varName = table.name;
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${varName}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${varName}.$inferInsert;`);
  }

  return lines.join('\n');
}

function generateSqliteTable(table: MergedTable, schema: MergedSchema): string {
  const lines: string[] = [];
  const { name, definition } = table;
  const dbTableName = definition.tableName || toSnakeCase(name);
  const hasCompositePK = !!definition.compositePrimaryKey?.length;
  const tableIndexes = table.indexes ?? [];

  lines.push(`export const ${name} = sqliteTable('${dbTableName}', {`);

  for (const [fieldName, field] of Object.entries(definition.fields)) {
    const colName = toSnakeCase(fieldName);
    lines.push(`  ${fieldName}: ${sqliteColumn(fieldName, colName, field, name, schema)},`);
  }

  const tableExtras: string[] = [];
  if (hasCompositePK) {
    const pkCols = (definition.compositePrimaryKey ?? []).map((f) => `table.${f}`).join(', ');
    tableExtras.push(`primaryKey({ columns: [${pkCols}] })`);
  }
  for (const idx of tableIndexes) {
    const cols = idx.columns.map((c) => `table.${c}`).join(', ');
    tableExtras.push(`index('${idx.name}').on(${cols})`);
  }

  if (tableExtras.length > 0) {
    lines.push(`}, (table) => [${tableExtras.join(', ')}]);`);
  } else {
    lines.push(`});`);
  }

  return lines.join('\n');
}

function sqliteColumn(
  fieldName: string,
  colName: string,
  field: PluginFieldAttribute,
  tableName: string,
  schema?: MergedSchema,
): string {
  let col: string;

  if (isEnumType(field.type)) {
    // SQLite: enums are just text columns
    col = `text('${colName}')`;
  } else {
    switch (field.type) {
      case 'string':
      case 'text':
        col = `text('${colName}')`;
        break;
      case 'uuid':
        col = `text('${colName}')`;
        break;
      case 'number':
      case 'bigint':
        col = `integer('${colName}')`;
        break;
      case 'boolean':
        col = `integer('${colName}', { mode: 'boolean' })`;
        break;
      case 'date':
        col = `text('${colName}')`;
        break;
      case 'json':
        col = `text('${colName}', { mode: 'json' })`;
        break;
      default:
        col = `text('${colName}')`;
    }
  }

  // Type annotation
  if (field.typeAnnotation) {
    col += `.$type<${field.typeAnnotation}>()`;
  }

  // Primary key
  if (field.primaryKey) {
    col += `.primaryKey()`;
  }

  // Not null
  if (field.required !== false) {
    col += `.notNull()`;
  }

  // Unique
  if (field.unique) {
    col += `.unique()`;
  }

  // Default value
  if (field.defaultValue !== undefined) {
    col += sqliteDefault(field.defaultValue, field.type, field.typeAnnotation);
  }

  // Foreign key
  if (field.references) {
    const refTable = findTableVarForDbName(field.references.table, schema);
    const refField = field.references.field;
    const onDelete = field.references.onDelete || 'no action';
    if (refTable === tableName) {
      col += `.references((): AnySQLiteColumn => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    } else {
      col += `.references(() => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    }
  }

  return col;
}

function sqliteDefault(
  value: string | number | boolean | 'uuid()' | 'now()',
  _type: PluginFieldType,
  typeAnnotation?: string,
): string {
  if (value === 'uuid()') {
    return `.$defaultFn(() => randomUUID())`;
  }
  if (value === 'now()') {
    return `.default(sql\`CURRENT_TIMESTAMP\`)`;
  }
  if (typeof value === 'boolean') {
    return `.default(${value})`;
  }
  if (typeof value === 'number') {
    return `.default(${value})`;
  }
  if (typeof value === 'string') {
    // For enum-backed columns we keep the type annotation via `import type`
    // but emit the default as a bare string literal — drizzle's column is
    // `text`, so `.default('PENDING')` is equivalent to
    // `.default(FlowRunStatus.PENDING)` and avoids a runtime import the
    // consumer's bundler would otherwise need to resolve.
    //
    // When the column carries a `.$type<T>()` annotation, a bare string
    // literal isn't assignable to the narrowed type (a nominal enum or a
    // JSON object/array shape). Emit the default as a first-class drizzle
    // `sql` raw default instead — `.default()` accepts `SQL<unknown>` for
    // any column type, so no cast or enum-member lookup is needed, and the
    // generated DDL is identical (`DEFAULT '<value>'`).
    return typeAnnotation ? `.default(sql\`'${value}'\`)` : `.default('${value}')`;
  }
  return '';
}

// =============================================================================
// PostgreSQL Generator
// =============================================================================

/**
 * Whether the postgres output will contain at least one `sql` raw default.
 * Only `.$type<T>()` columns get one, and enum columns are typed by pgEnum
 * rather than `$type<>()` — so this mirrors the gate in `postgresColumn`.
 * Used to keep `sql` out of the import list when nothing references it.
 */
function postgresUsesSqlDefault(schema: MergedSchema): boolean {
  return schema.tables.some((table) =>
    Object.values(table.definition.fields).some(
      (field) =>
        typeof field.defaultValue === 'string' &&
        field.defaultValue !== 'uuid()' &&
        field.defaultValue !== 'now()' &&
        field.typeAnnotation !== undefined &&
        !isEnumType(field.type),
    ),
  );
}

export function generatePostgresSchema(schema: MergedSchema): string {
  const lines: string[] = [];
  const hasIndexes = schemaHasInjectedIndexes(schema);
  const usesSql = postgresUsesSqlDefault(schema);

  // Header
  lines.push(`// PostgreSQL schema for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit manually. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(``);
  lines.push(`import {`);
  lines.push(`  pgTable,`);
  lines.push(`  text,`);
  lines.push(`  integer,`);
  lines.push(`  boolean,`);
  lines.push(`  timestamp,`);
  lines.push(`  json,`);
  lines.push(`  pgEnum,`);
  lines.push(`  uuid,`);
  lines.push(`  bigint,`);
  if (hasIndexes) {
    lines.push(`  index,`);
  }
  lines.push(`  type AnyPgColumn,`);
  lines.push(`  primaryKey,`);
  lines.push(`} from 'drizzle-orm/pg-core';`);
  lines.push(`import { relations${usesSql ? ', sql' : ''} } from 'drizzle-orm';`);
  lines.push(`import { randomUUID } from 'crypto';`);
  lines.push(``);

  // Enum columns use pgEnum (not .$type<>()), so their type annotations are unused
  const enumTypeNames = collectEnumTypeAnnotations(schema);

  // Type imports from @flowlib/core/types
  const typeImport = collectTypeImportsForAppend(schema, enumTypeNames);
  if (typeImport) {
    lines.push(typeImport);
  }

  // Local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema, enumTypeNames);
  if (localAliases.length > 0) {
    for (const alias of localAliases) {
      lines.push(alias);
    }
  }
  lines.push(``);

  // Enums
  const enums = collectEnums(schema);
  if (enums.length > 0) {
    lines.push(`// =============================================================================`);
    lines.push(`// Enums`);
    lines.push(`// =============================================================================`);
    lines.push(``);
    for (const e of enums) {
      lines.push(
        `export const ${e.varName} = pgEnum('${e.dbName}', [${e.values.map((v) => `'${v}'`).join(', ')}]);`,
      );
    }
    lines.push(``);
  }

  // Tables
  lines.push(`// =============================================================================`);
  lines.push(`// Tables`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  for (const table of schema.tables) {
    lines.push(generatePostgresTable(table, schema, enums));
    lines.push(``);
  }

  // Relations
  lines.push(`// =============================================================================`);
  lines.push(`// Relations`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  lines.push(generateRelations(schema));

  // Type exports
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Type exports`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  for (const table of schema.tables) {
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${table.name}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${table.name}.$inferInsert;`);
  }

  return lines.join('\n');
}

function generatePostgresTable(table: MergedTable, schema: MergedSchema, enums: EnumDef[]): string {
  const lines: string[] = [];
  const { name, definition } = table;
  const dbTableName = definition.tableName || toSnakeCase(name);
  const hasCompositePK = !!definition.compositePrimaryKey?.length;
  const tableIndexes = table.indexes ?? [];

  lines.push(`export const ${name} = pgTable('${dbTableName}', {`);

  for (const [fieldName, field] of Object.entries(definition.fields)) {
    const colName = toSnakeCase(fieldName);
    lines.push(
      `  ${fieldName}: ${postgresColumn(fieldName, colName, field, name, enums, schema)},`,
    );
  }

  const tableExtras: string[] = [];
  if (hasCompositePK) {
    const pkCols = (definition.compositePrimaryKey ?? []).map((f) => `table.${f}`).join(', ');
    tableExtras.push(`primaryKey({ columns: [${pkCols}] })`);
  }
  for (const idx of tableIndexes) {
    const cols = idx.columns.map((c) => `table.${c}`).join(', ');
    tableExtras.push(`index('${idx.name}').on(${cols})`);
  }

  if (tableExtras.length > 0) {
    lines.push(`}, (table) => [${tableExtras.join(', ')}]);`);
  } else {
    lines.push(`});`);
  }

  return lines.join('\n');
}

function postgresColumn(
  fieldName: string,
  colName: string,
  field: PluginFieldAttribute,
  tableName: string,
  enums: EnumDef[],
  schema?: MergedSchema,
): string {
  let col: string;

  if (isEnumType(field.type)) {
    const eVar = enums.find((e) => e.tableName === tableName && e.fieldName === fieldName)?.varName;
    col = eVar ? `${eVar}('${colName}')` : `text('${colName}')`;
  } else {
    switch (field.type) {
      case 'string':
      case 'text':
        col = `text('${colName}')`;
        break;
      case 'uuid':
        col = `uuid('${colName}')`;
        break;
      case 'number':
        col = `integer('${colName}')`;
        break;
      case 'bigint':
        col = `bigint('${colName}', { mode: 'bigint' })`;
        break;
      case 'boolean':
        col = `boolean('${colName}')`;
        break;
      case 'date':
        col = `timestamp('${colName}')`;
        break;
      case 'json':
        col = `json('${colName}')`;
        break;
      default:
        col = `text('${colName}')`;
    }
  }

  // Type annotation (skip for enum columns — pgEnum already types them correctly)
  if (field.typeAnnotation && !isEnumType(field.type)) {
    col += `.$type<${field.typeAnnotation}>()`;
  }

  // Primary key
  if (field.primaryKey) {
    col += `.primaryKey()`;
  }

  // Not null
  if (field.required !== false) {
    col += `.notNull()`;
  }

  // Unique
  if (field.unique) {
    col += `.unique()`;
  }

  // Default value. Only pass the type annotation when the column actually
  // carries a `.$type<T>()` — enum columns are typed by pgEnum (see above), so
  // a bare string literal is already assignable and the `sql` form would be
  // spurious.
  if (field.defaultValue !== undefined) {
    col += postgresDefault(
      field.defaultValue,
      field.type,
      isEnumType(field.type) ? undefined : field.typeAnnotation,
    );
  }

  // Foreign key
  if (field.references) {
    const refTable = findTableVarForDbName(field.references.table, schema);
    const refField = field.references.field;
    const onDelete = field.references.onDelete || 'no action';
    if (refTable === tableName) {
      col += `.references((): AnyPgColumn => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    } else {
      col += `.references(() => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    }
  }

  return col;
}

function postgresDefault(
  value: string | number | boolean | 'uuid()' | 'now()',
  _type: PluginFieldType,
  typeAnnotation?: string,
): string {
  if (value === 'uuid()') {
    return `.$default(() => randomUUID())`;
  }
  if (value === 'now()') {
    return `.defaultNow()`;
  }
  if (typeof value === 'boolean') {
    return `.default(${value})`;
  }
  if (typeof value === 'number') {
    return `.default(${value})`;
  }
  if (typeof value === 'string') {
    // `sql` raw default for `.$type<T>()` columns (enum / JSON shape) — it
    // satisfies `.default()`'s `SQL<unknown>` overload without a cast; see
    // `sqliteDefault`.
    return typeAnnotation ? `.default(sql\`'${value}'\`)` : `.default('${value}')`;
  }
  return '';
}

// =============================================================================
// MySQL Generator
// =============================================================================

export function generateMysqlSchema(schema: MergedSchema): string {
  const lines: string[] = [];
  const hasIndexes = schemaHasInjectedIndexes(schema);

  // Header
  lines.push(`// MySQL schema for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit manually. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(``);
  lines.push(`import {`);
  lines.push(`  mysqlTable,`);
  lines.push(`  varchar,`);
  lines.push(`  text,`);
  lines.push(`  int,`);
  lines.push(`  bigint,`);
  lines.push(`  boolean,`);
  lines.push(`  timestamp,`);
  lines.push(`  json,`);
  lines.push(`  mysqlEnum,`);
  if (hasIndexes) {
    lines.push(`  index,`);
  }
  lines.push(`  type AnyMySqlColumn,`);
  lines.push(`  primaryKey,`);
  lines.push(`} from 'drizzle-orm/mysql-core';`);
  lines.push(`import { relations, sql } from 'drizzle-orm';`);
  lines.push(`import { randomUUID } from 'crypto';`);
  lines.push(``);

  // Type imports from @flowlib/core/types
  const typeImport = collectTypeImportsForAppend(schema);
  if (typeImport) {
    lines.push(typeImport);
  }

  // Local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema);
  if (localAliases.length > 0) {
    for (const alias of localAliases) {
      lines.push(alias);
    }
  }
  lines.push(``);

  // Tables
  lines.push(`// =============================================================================`);
  lines.push(`// Tables`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  for (const table of schema.tables) {
    lines.push(generateMysqlTable(table, schema));
    lines.push(``);
  }

  // Relations
  lines.push(`// =============================================================================`);
  lines.push(`// Relations`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  lines.push(generateRelations(schema));

  // Type exports
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Type exports`);
  lines.push(`// =============================================================================`);
  lines.push(``);
  for (const table of schema.tables) {
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${table.name}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${table.name}.$inferInsert;`);
  }

  return lines.join('\n');
}

function generateMysqlTable(table: MergedTable, schema: MergedSchema): string {
  // schema is used by mysqlColumn for FK reference resolution
  const lines: string[] = [];
  const { name, definition } = table;
  const dbTableName = definition.tableName || toSnakeCase(name);
  const hasCompositePK = !!definition.compositePrimaryKey?.length;
  const tableIndexes = table.indexes ?? [];

  lines.push(`export const ${name} = mysqlTable('${dbTableName}', {`);

  for (const [fieldName, field] of Object.entries(definition.fields)) {
    const colName = toSnakeCase(fieldName);
    lines.push(`  ${fieldName}: ${mysqlColumn(fieldName, colName, field, name, schema)},`);
  }

  const tableExtras: string[] = [];
  if (hasCompositePK) {
    const pkCols = (definition.compositePrimaryKey ?? []).map((f) => `table.${f}`).join(', ');
    tableExtras.push(`primaryKey({ columns: [${pkCols}] })`);
  }
  for (const idx of tableIndexes) {
    const cols = idx.columns.map((c) => `table.${c}`).join(', ');
    tableExtras.push(`index('${idx.name}').on(${cols})`);
  }

  if (tableExtras.length > 0) {
    lines.push(`}, (table) => [${tableExtras.join(', ')}]);`);
  } else {
    lines.push(`});`);
  }

  return lines.join('\n');
}

function mysqlColumn(
  fieldName: string,
  colName: string,
  field: PluginFieldAttribute,
  tableName: string,
  schema?: MergedSchema,
): string {
  const maxLen = field.maxLength ?? 255;
  let col: string;

  if (isEnumType(field.type)) {
    const enumValues = field.type.map((v) => `'${v}'`).join(', ');
    col = `mysqlEnum('${colName}', [${enumValues}])`;
  } else {
    switch (field.type) {
      case 'string':
        col = `varchar('${colName}', { length: ${maxLen} })`;
        break;
      case 'text':
        col = `text('${colName}')`;
        break;
      case 'uuid':
        col = `varchar('${colName}', { length: 36 })`;
        break;
      case 'number':
        col = `int('${colName}')`;
        break;
      case 'bigint':
        col = `bigint('${colName}', { mode: 'bigint' })`;
        break;
      case 'boolean':
        col = `boolean('${colName}')`;
        break;
      case 'date':
        col = `timestamp('${colName}')`;
        break;
      case 'json':
        col = `json('${colName}')`;
        break;
      default:
        col = `varchar('${colName}', { length: ${maxLen} })`;
    }
  }

  // Type annotation (skip for enum columns — mysqlEnum already types them correctly)
  if (field.typeAnnotation && !isEnumType(field.type)) {
    col += `.$type<${field.typeAnnotation}>()`;
  }

  // Primary key
  if (field.primaryKey) {
    col += `.primaryKey()`;
  }

  // Not null
  if (field.required !== false) {
    col += `.notNull()`;
  }

  // Unique
  if (field.unique) {
    col += `.unique()`;
  }

  // Default value
  if (field.defaultValue !== undefined) {
    col += mysqlDefault(field.defaultValue, field.type, field.typeAnnotation);
  }

  // Foreign key
  if (field.references) {
    const refTable = findTableVarForDbName(field.references.table, schema);
    const refField = field.references.field;
    const onDelete = field.references.onDelete || 'no action';
    if (refTable === tableName) {
      col += `.references((): AnyMySqlColumn => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    } else {
      col += `.references(() => ${refTable}.${refField}, { onDelete: '${onDelete}' })`;
    }
  }

  return col;
}

function mysqlDefault(
  value: string | number | boolean | 'uuid()' | 'now()',
  type: PluginFieldType,
  typeAnnotation?: string,
): string {
  if (value === 'uuid()') {
    return `.$defaultFn(() => randomUUID())`;
  }
  if (value === 'now()') {
    return `.default(sql\`CURRENT_TIMESTAMP\`)`;
  }
  if (typeof value === 'boolean') {
    return `.default(${value})`;
  }
  if (typeof value === 'number') {
    return `.default(${value})`;
  }
  if (typeof value === 'string') {
    // MySQL rejects a literal DEFAULT on BLOB/TEXT/JSON columns (error 1101).
    // Since 8.0.13 those columns accept a default only as a parenthesized
    // expression, so emit `DEFAULT ('<value>')` for json.
    if (type === 'json') {
      return `.default(sql\`('${value}')\`)`;
    }
    // `sql` raw default for `.$type<T>()` columns (enum / JSON shape) — it
    // satisfies `.default()`'s `SQL<unknown>` overload without a cast; see
    // `sqliteDefault`.
    return typeAnnotation ? `.default(sql\`'${value}'\`)` : `.default('${value}')`;
  }
  return '';
}

// =============================================================================
// Append-mode Generators
// =============================================================================
//
// These produce only the table definitions, relations, and type exports
// for appending to an existing Drizzle schema file. They include:
//   - A commented section marker
//   - Additional imports needed (inserted at top when used by CLI)
//   - Table definitions
//   - Relations
//   - Type exports
//
// The CLI handles merging the import lines into the existing file.
// This approach mirrors how better-auth generates schema code that can
// be appended to the user's existing schema file.

export interface AppendSchemaResult {
  /** Import statements that need to be added (CLI merges these) */
  imports: string[];
  /** Table + relation + type code to append */
  code: string;
}

export function generateSqliteSchemaAppend(schema: MergedSchema): AppendSchemaResult {
  const runtimeImports = collectSqliteRuntimeDefaultImports(schema);
  const hasIndexes = schemaHasInjectedIndexes(schema);
  const imports = [
    `import { sqliteTable, text, integer, ${hasIndexes ? 'index, ' : ''}primaryKey, type AnySQLiteColumn } from 'drizzle-orm/sqlite-core';`,
    `import { relations, sql } from 'drizzle-orm';`,
    `import { randomUUID } from 'crypto';`,
  ];

  // Type-only — bundlers in append-mode consumer repos may not be able to
  // resolve `@flowlib/action-kit` at runtime. Defaults are inlined as string
  // literals (see `sqliteDefault`) so no value import is needed.
  for (const [importPath, identifiers] of runtimeImports.entries()) {
    imports.push(
      `import type { ${Array.from(identifiers).sort().join(', ')} } from '${importPath}';`,
    );
  }

  // Collect names already imported as runtime values to avoid duplicates in type import
  const runtimeImportedNames = new Set<string>();
  for (const identifiers of runtimeImports.values()) {
    for (const id of identifiers) {
      runtimeImportedNames.add(id);
    }
  }

  const typeImport = collectTypeImportsForAppend(schema, runtimeImportedNames);
  if (typeImport) {
    imports.push(typeImport);
  }

  // Generate local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema, runtimeImportedNames);

  const lines: string[] = [];
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Flowlib tables — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit below this line. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  if (localAliases.length > 0) {
    for (const alias of localAliases) {
      lines.push(alias);
    }
    lines.push(``);
  }

  for (const table of schema.tables) {
    lines.push(generateSqliteTable(table, schema));
    lines.push(``);
  }

  lines.push(generateRelations(schema));
  lines.push(``);

  for (const table of schema.tables) {
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${table.name}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${table.name}.$inferInsert;`);
  }

  return { imports, code: lines.join('\n') };
}

export function generatePostgresSchemaAppend(schema: MergedSchema): AppendSchemaResult {
  const hasIndexes = schemaHasInjectedIndexes(schema);
  const imports = [
    `import { pgTable, text, integer, boolean, timestamp, json, pgEnum, uuid, bigint, ${hasIndexes ? 'index, ' : ''}primaryKey, type AnyPgColumn } from 'drizzle-orm/pg-core';`,
    `import { relations${postgresUsesSqlDefault(schema) ? ', sql' : ''} } from 'drizzle-orm';`,
    `import { randomUUID } from 'crypto';`,
  ];

  // Enum columns use pgEnum (not .$type<>()), so their type annotations are unused
  const enumTypeNames = collectEnumTypeAnnotations(schema);
  const typeImport = collectTypeImportsForAppend(schema, enumTypeNames);
  if (typeImport) {
    imports.push(typeImport);
  }

  // Generate local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema, enumTypeNames);

  const lines: string[] = [];
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Flowlib tables — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit below this line. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  if (localAliases.length > 0) {
    for (const alias of localAliases) {
      lines.push(alias);
    }
    lines.push(``);
  }

  const enums = collectEnums(schema);
  if (enums.length > 0) {
    for (const e of enums) {
      lines.push(
        `export const ${e.varName} = pgEnum('${e.dbName}', [${e.values.map((v) => `'${v}'`).join(', ')}]);`,
      );
    }
    lines.push(``);
  }

  for (const table of schema.tables) {
    lines.push(generatePostgresTable(table, schema, enums));
    lines.push(``);
  }

  lines.push(generateRelations(schema));
  lines.push(``);

  for (const table of schema.tables) {
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${table.name}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${table.name}.$inferInsert;`);
  }

  return { imports, code: lines.join('\n') };
}

export function generateMysqlSchemaAppend(schema: MergedSchema): AppendSchemaResult {
  const hasIndexes = schemaHasInjectedIndexes(schema);
  const imports = [
    `import { mysqlTable, varchar, text, int, bigint, boolean, timestamp, json, mysqlEnum, ${hasIndexes ? 'index, ' : ''}primaryKey, type AnyMySqlColumn } from 'drizzle-orm/mysql-core';`,
    `import { relations, sql } from 'drizzle-orm';`,
    `import { randomUUID } from 'crypto';`,
  ];

  // Enum columns use mysqlEnum (not .$type<>()), so their type annotations are unused
  const enumTypeNames = collectEnumTypeAnnotations(schema);
  const typeImport = collectTypeImportsForAppend(schema, enumTypeNames);
  if (typeImport) {
    imports.push(typeImport);
  }

  // Generate local type aliases for plugin-defined types not in @flowlib/core/types
  const localAliases = collectLocalTypeAliases(schema, enumTypeNames);

  const lines: string[] = [];
  lines.push(``);
  lines.push(`// =============================================================================`);
  lines.push(`// Flowlib tables — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`// Do not edit below this line. Run \`npx flowlib-cli generate\` to regenerate.`);
  lines.push(`// =============================================================================`);
  lines.push(``);

  if (localAliases.length > 0) {
    for (const alias of localAliases) {
      lines.push(alias);
    }
    lines.push(``);
  }

  for (const table of schema.tables) {
    lines.push(generateMysqlTable(table, schema));
    lines.push(``);
  }

  lines.push(generateRelations(schema));
  lines.push(``);

  for (const table of schema.tables) {
    const typeName = capitalize(table.name);
    lines.push(`export type ${typeName} = typeof ${table.name}.$inferSelect;`);
    lines.push(`export type New${typeName} = typeof ${table.name}.$inferInsert;`);
  }

  return { imports, code: lines.join('\n') };
}

// =============================================================================
// Relation Generator (shared across dialects)
// =============================================================================

function generateRelations(schema: MergedSchema): string {
  const lines: string[] = [];

  // Build a map of FK references: which tables reference which
  const refs: { from: string; fromField: string; to: string; toField: string }[] = [];

  for (const table of schema.tables) {
    for (const [fieldName, field] of Object.entries(table.definition.fields)) {
      if (field.references) {
        const toTableVar = findTableVarForDbName(field.references.table, schema);
        refs.push({
          from: table.name,
          fromField: fieldName,
          to: toTableVar,
          toField: field.references.field,
        });
      }
    }
  }

  for (const table of schema.tables) {
    // Find "one" relations (this table has FK → other table)
    const oneRels = refs.filter((r) => r.from === table.name);
    // Find "many" relations (other tables have FK → this table)
    const manyRels = refs.filter((r) => r.to === table.name);

    if (oneRels.length === 0 && manyRels.length === 0) {
      continue;
    }

    const relParts: string[] = [];
    const destructure: string[] = [];

    if (oneRels.length > 0) {
      destructure.push('one');
    }
    if (manyRels.length > 0) {
      destructure.push('many');
    }

    for (const rel of oneRels) {
      // Derive a relation name from the FK field: "flowId" → "flow"
      const relName = rel.fromField.replace(/Id$/, '').replace(/(_id)$/, '');
      relParts.push(
        `  ${relName}: one(${rel.to}, { fields: [${table.name}.${rel.fromField}], references: [${rel.to}.${rel.toField}] }),`,
      );
    }

    for (const rel of manyRels) {
      // Derive a relation name from the source table
      const relName = rel.from;
      // Avoid duplicates (e.g., multiple FKs from same table)
      relParts.push(`  ${relName}: many(${rel.from}),`);
    }

    lines.push(
      `export const ${table.name}Relations = relations(${table.name}, ({ ${destructure.join(', ')} }) => ({`,
    );
    lines.push(relParts.join('\n'));
    lines.push(`}));`);
    lines.push(``);
  }

  return lines.join('\n');
}

// =============================================================================
// Enum Collection
// =============================================================================

interface EnumDef {
  varName: string;
  dbName: string;
  values: string[];
  tableName: string;
  fieldName: string;
}

function collectEnums(schema: MergedSchema): EnumDef[] {
  const enums: EnumDef[] = [];

  for (const table of schema.tables) {
    for (const [fieldName, field] of Object.entries(table.definition.fields)) {
      if (isEnumType(field.type)) {
        const varName = enumVarName(table.name, fieldName);
        const dbName = `${toSnakeCase(table.name)}_${toSnakeCase(fieldName)}`;
        enums.push({
          varName,
          dbName,
          values: field.type,
          tableName: table.name,
          fieldName,
        });
      }
    }
  }

  return enums;
}

// =============================================================================
// Type Annotation Collection
// =============================================================================

function collectTypeAnnotations(schema: MergedSchema): Set<string> {
  const annotations = new Set<string>();
  for (const table of schema.tables) {
    for (const field of Object.values(table.definition.fields)) {
      if (field.typeAnnotation) {
        annotations.add(field.typeAnnotation);
      }
    }
  }
  return annotations;
}

/**
 * Collect type annotations that belong to enum columns.
 * These are handled by pgEnum/mysqlEnum and don't need .$type<>() or type imports.
 */
function collectEnumTypeAnnotations(schema: MergedSchema): Set<string> {
  const enumAnnotations = new Set<string>();
  for (const table of schema.tables) {
    for (const field of Object.values(table.definition.fields)) {
      if (field.typeAnnotation && isEnumType(field.type)) {
        enumAnnotations.add(field.typeAnnotation);
      }
    }
  }
  return enumAnnotations;
}

/**
 * Built-in TypeScript types that don't need an import statement OR a local
 * `type X = string;` alias. If we emitted an alias for `unknown`, the
 * generated file would shadow the global `unknown` type and fail to compile.
 *
 * Includes:
 *   - Bare TS primitives (`unknown`, `any`, `string`, `number`, `boolean`,
 *     `null`, `undefined`, `void`, `never`, `object`)
 *   - Primitive arrays
 *   - `Record<>` utility types
 *   - Inline union literals
 */
const BUILTIN_TS_TYPES = new Set([
  'unknown',
  'any',
  'string',
  'number',
  'boolean',
  'null',
  'undefined',
  'void',
  'never',
  'object',
]);

/** Plain TS identifier — what `type X = …` requires on the LHS. */
const IDENTIFIER_RE = /^[A-Za-z_$][A-Za-z0-9_$]*$/;

function isBuiltinType(annotation: string): boolean {
  // Bare primitives
  if (BUILTIN_TS_TYPES.has(annotation)) {
    return true;
  }
  // Any array form — `T[]` for any T (including `unknown[]`, `MyType[]`).
  if (annotation.endsWith('[]')) {
    return true;
  }
  // Generic utility types — `Record<…>`, `Array<…>`, `Set<…>`, `Map<…>`, etc.
  // These can't appear on the LHS of `type X = …`, so we skip aliasing them.
  if (/^[A-Za-z_$][A-Za-z0-9_$]*</.test(annotation)) {
    return true;
  }
  // Inline object literal types (e.g. `{ lines: number; bytes: number }`).
  if (annotation.trimStart().startsWith('{')) {
    return true;
  }
  // Inline union literal types (e.g., "'user' | 'assistant' | 'system' | 'tool'")
  if (annotation.includes("'") && annotation.includes('|')) {
    return true;
  }
  // Anything that isn't a plain identifier can't be a `type X = string` LHS,
  // so it's safer to treat it as builtin/inline and skip alias generation.
  if (!IDENTIFIER_RE.test(annotation)) {
    return true;
  }
  return false;
}

/**
 * Types that are known to be exported from `@flowlib/core/types`.
 * Only these will be included in `import type { ... } from '@flowlib/core/types'`.
 * Plugin-defined types not in this set get generated as local type aliases.
 */
const CORE_TYPE_EXPORTS = new Set([
  'BatchProvider',
  'BatchStatus',
  'CredentialAuthType',
  'CredentialConfig',
  'CredentialType',
  'FlowRunStatus',
  'FlowlibDefinitionRuntime',
  'JSONValue',
  'NodeExecutionStatus',
  'TriggerType',
]);

/**
 * Generate a type import line for `@flowlib/core/types` based on the type annotations
 * used in the schema. This is needed in append mode where the generated code lives
 * in the user's project and can't use internal `src/` paths.
 *
 * Types not in CORE_TYPE_EXPORTS are excluded — they should be handled
 * separately via `collectLocalTypeAliases`.
 */
function collectTypeImportsForAppend(
  schema: MergedSchema,
  runtimeImportedNames?: Set<string>,
): string | null {
  const annotations = collectTypeAnnotations(schema);
  const importable = Array.from(annotations)
    .filter((a) => !isBuiltinType(a))
    .filter((a) => !runtimeImportedNames?.has(a))
    .filter((a) => CORE_TYPE_EXPORTS.has(a))
    .sort();

  if (importable.length === 0) {
    return null;
  }
  return `import type { ${importable.join(', ')} } from '@flowlib/core/types';`;
}

/**
 * Generate local `type X = string;` aliases for type annotations that are NOT
 * available from `@flowlib/core/types` (i.e. plugin-defined types).
 *
 * These appear as simple string aliases so the generated schema compiles
 * without requiring additional imports from plugin packages.
 */
function collectLocalTypeAliases(
  schema: MergedSchema,
  runtimeImportedNames?: Set<string>,
): string[] {
  const annotations = collectTypeAnnotations(schema);
  return Array.from(annotations)
    .filter((a) => !isBuiltinType(a))
    .filter((a) => !runtimeImportedNames?.has(a))
    .filter((a) => !CORE_TYPE_EXPORTS.has(a))
    .sort()
    .map((a) => `type ${a} = string;`);
}

// =============================================================================
// Shared Helpers
// =============================================================================

/**
 * True iff any merged table carries one or more transform-injected indexes.
 * Drives whether the dialect import line includes `index`.
 */
function schemaHasInjectedIndexes(schema: MergedSchema): boolean {
  for (const table of schema.tables) {
    if (table.indexes && table.indexes.length > 0) {
      return true;
    }
  }
  return false;
}

/**
 * Build a map from DB table name → JS variable name from the schema.
 */
function buildTableMap(schema: MergedSchema): Map<string, string> {
  const map = new Map<string, string>();
  for (const table of schema.tables) {
    const dbName = table.definition.tableName || toSnakeCase(table.name);
    map.set(dbName, table.name);
    // Also map by logical name for convenience
    map.set(table.name, table.name);
  }
  return map;
}

/**
 * Find the JS variable name for a given DB table name.
 * Uses the schema to do a proper lookup (e.g., 'execution_traces' → 'nodeExecutions').
 * Falls back to camelCase heuristic if no schema provided.
 */
function findTableVarForDbName(dbName: string, schema?: MergedSchema): string {
  if (schema) {
    const tableMap = buildTableMap(schema);
    const found = tableMap.get(dbName);
    if (found) {
      return found;
    }
  }
  // Fallback: convert snake_case to camelCase
  if (dbName.includes('_')) {
    return dbName.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
  }
  return dbName;
}

function capitalize(str: string): string {
  return str.charAt(0).toUpperCase() + str.slice(1);
}

// =============================================================================
// Raw SQL Generators
// =============================================================================

/**
 * Generate raw SQL CREATE TABLE statements for SQLite.
 */
export function generateSqliteRawSql(schema: MergedSchema): string {
  const lines: string[] = [];
  lines.push(`-- SQLite migration for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`-- Run against your SQLite database to create the required tables.`);
  lines.push(``);

  for (const table of schema.tables) {
    lines.push(generateRawSqlTable(table, 'sqlite'));
    lines.push(``);
  }

  return lines.join('\n');
}

/**
 * Generate raw SQL CREATE TABLE statements for PostgreSQL.
 */
export function generatePostgresRawSql(schema: MergedSchema): string {
  const lines: string[] = [];
  lines.push(`-- PostgreSQL migration for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`-- Run against your PostgreSQL database to create the required tables.`);
  lines.push(``);

  // Collect enums for PostgreSQL
  for (const table of schema.tables) {
    for (const [fieldName, field] of Object.entries(table.definition.fields)) {
      if (isEnumType(field.type)) {
        const eName = `${table.definition.tableName || toSnakeCase(table.name)}_${toSnakeCase(fieldName)}`;
        lines.push(`CREATE TYPE ${eName} AS ENUM (${field.type.map((v) => `'${v}'`).join(', ')});`);
      }
    }
  }
  if (lines.length > 3) {
    lines.push(``);
  }

  for (const table of schema.tables) {
    lines.push(generateRawSqlTable(table, 'postgresql'));
    lines.push(``);
  }

  return lines.join('\n');
}

/**
 * Generate raw SQL CREATE TABLE statements for MySQL.
 */
export function generateMysqlRawSql(schema: MergedSchema): string {
  const lines: string[] = [];
  lines.push(`-- MySQL migration for Flowlib — AUTO-GENERATED by @flowlib/cli`);
  lines.push(`-- Run against your MySQL database to create the required tables.`);
  lines.push(``);

  for (const table of schema.tables) {
    lines.push(generateRawSqlTable(table, 'mysql'));
    lines.push(``);
  }

  return lines.join('\n');
}

function generateRawSqlTable(
  table: MergedTable,
  dialect: 'sqlite' | 'postgresql' | 'mysql',
): string {
  const lines: string[] = [];
  const { definition } = table;
  const dbTableName = definition.tableName || toSnakeCase(table.name);
  const hasCompositePK = !!definition.compositePrimaryKey?.length;

  lines.push(`CREATE TABLE IF NOT EXISTS ${quoteIdent(dbTableName, dialect)} (`);

  const colDefs: string[] = [];
  const constraints: string[] = [];

  for (const [fieldName, field] of Object.entries(definition.fields)) {
    const colName = toSnakeCase(fieldName);
    const colType = rawSqlColumnType(field, dialect, dbTableName, colName);
    let colDef = `  ${quoteIdent(colName, dialect)} ${colType}`;

    if (field.primaryKey && !hasCompositePK) {
      colDef += ' PRIMARY KEY';
    }
    if (field.required !== false && !field.primaryKey) {
      colDef += ' NOT NULL';
    }
    if (field.unique) {
      colDef += ' UNIQUE';
    }
    const def = rawSqlDefault(field, dialect);
    if (def) {
      colDef += ` DEFAULT ${def}`;
    }
    if (field.references) {
      const refTableName = field.references.table;
      const refField = toSnakeCase(field.references.field);
      const onDelete = (field.references.onDelete || 'NO ACTION').toUpperCase().replace('_', ' ');
      constraints.push(
        `  FOREIGN KEY (${quoteIdent(colName, dialect)}) REFERENCES ${quoteIdent(refTableName, dialect)}(${quoteIdent(refField, dialect)}) ON DELETE ${onDelete}`,
      );
    }
    colDefs.push(colDef);
  }

  if (hasCompositePK) {
    const pkCols = (definition.compositePrimaryKey ?? [])
      .map((f) => quoteIdent(toSnakeCase(f), dialect))
      .join(', ');
    constraints.push(`  PRIMARY KEY (${pkCols})`);
  }

  const allParts = [...colDefs, ...constraints];
  lines.push(allParts.join(',\n'));
  lines.push(`);`);

  // Transform-injected indexes
  for (const idx of table.indexes ?? []) {
    const cols = idx.columns.map((c) => quoteIdent(toSnakeCase(c), dialect)).join(', ');
    lines.push(
      `CREATE INDEX IF NOT EXISTS ${quoteIdent(idx.name, dialect)} ON ${quoteIdent(dbTableName, dialect)} (${cols});`,
    );
  }

  return lines.join('\n');
}

function quoteIdent(name: string, dialect: 'sqlite' | 'postgresql' | 'mysql'): string {
  if (dialect === 'mysql') {
    return `\`${name}\``;
  }
  return `"${name}"`;
}

function rawSqlColumnType(
  field: PluginFieldAttribute,
  dialect: 'sqlite' | 'postgresql' | 'mysql',
  tableName: string,
  colName: string,
): string {
  if (isEnumType(field.type)) {
    if (dialect === 'sqlite') {
      return 'TEXT';
    }
    if (dialect === 'postgresql') {
      return `${tableName}_${colName}`;
    }
    // MySQL inline enum
    return `ENUM(${field.type.map((v) => `'${v}'`).join(', ')})`;
  }

  const typeMap: Record<string, Record<string, string>> = {
    sqlite: {
      string: 'TEXT',
      text: 'TEXT',
      uuid: 'TEXT',
      number: 'INTEGER',
      bigint: 'INTEGER',
      boolean: 'INTEGER',
      date: 'TEXT',
      json: 'TEXT',
    },
    postgresql: {
      string: 'TEXT',
      text: 'TEXT',
      uuid: 'UUID',
      number: 'INTEGER',
      bigint: 'BIGINT',
      boolean: 'BOOLEAN',
      date: 'TIMESTAMP WITH TIME ZONE',
      json: 'JSONB',
    },
    mysql: {
      string: `VARCHAR(${field.maxLength || 255})`,
      text: 'TEXT',
      uuid: 'VARCHAR(36)',
      number: 'INT',
      bigint: 'BIGINT',
      boolean: 'BOOLEAN',
      date: 'TIMESTAMP',
      json: 'JSON',
    },
  };

  const t = typeof field.type === 'string' ? field.type : 'text';
  return typeMap[dialect][t] || 'TEXT';
}

function rawSqlDefault(
  field: PluginFieldAttribute,
  dialect: 'sqlite' | 'postgresql' | 'mysql',
): string | null {
  if (field.defaultValue === undefined) {
    return null;
  }
  if (field.defaultValue === 'uuid()') {
    if (dialect === 'postgresql') {
      return `gen_random_uuid()`;
    }
    // SQLite/MySQL: no built-in UUID default; omit (app generates)
    return null;
  }
  if (field.defaultValue === 'now()') {
    if (dialect === 'sqlite') {
      return `CURRENT_TIMESTAMP`;
    }
    if (dialect === 'postgresql') {
      return `NOW()`;
    }
    return `CURRENT_TIMESTAMP`;
  }
  if (typeof field.defaultValue === 'boolean') {
    if (dialect === 'sqlite') {
      return field.defaultValue ? '1' : '0';
    }
    return field.defaultValue ? 'TRUE' : 'FALSE';
  }
  if (typeof field.defaultValue === 'number') {
    return String(field.defaultValue);
  }
  if (typeof field.defaultValue === 'string') {
    return `'${field.defaultValue}'`;
  }
  return null;
}
