'use client';

import { memo } from 'react';
import { cn } from '../../lib/utils';
import { useOptionalTheme } from '../../contexts/ThemeProvider';
import { FLOWLIB_ICON_DARK_SVG, FLOWLIB_ICON_LIGHT_SVG } from '../../assets/flowlib-branding';

export interface FlowlibLogoProps {
  className?: string;
  iconClassName?: string;
  labelClassName?: string;
  showLabel?: boolean;
  label?: string;
  variant?: 'theme' | 'dark' | 'light';
}

function resolveLogoMarkup(variant: 'theme' | 'dark' | 'light', resolvedTheme: 'dark' | 'light') {
  if (variant === 'dark') {
    return FLOWLIB_ICON_DARK_SVG;
  }

  if (variant === 'light') {
    return FLOWLIB_ICON_LIGHT_SVG;
  }

  return resolvedTheme === 'dark' ? FLOWLIB_ICON_DARK_SVG : FLOWLIB_ICON_LIGHT_SVG;
}

export const FlowlibLogo = memo(function FlowlibLogo({
  className,
  iconClassName,
  labelClassName,
  showLabel = false,
  label = 'Flowlib',
  variant = 'theme',
}: FlowlibLogoProps) {
  const theme = useOptionalTheme();
  const resolvedTheme = theme?.resolvedTheme ?? 'light';
  const svgMarkup = resolveLogoMarkup(variant, resolvedTheme);

  return (
    <div className={cn('flex items-center gap-3', className)}>
      <span
        aria-hidden="true"
        className={cn(
          'flex shrink-0 items-center justify-center h-6 [&>svg]:h-full [&>svg]:w-full',
          iconClassName,
        )}
        style={{ aspectRatio: '109 / 209' }}
        dangerouslySetInnerHTML={{ __html: svgMarkup }}
      />
      {showLabel ? (
        <span className={cn('text-lg font-semibold tracking-tight', labelClassName)}>{label}</span>
      ) : null}
    </div>
  );
});

FlowlibLogo.displayName = 'FlowlibLogo';
