...
INVECT_ENCRYPTION_KEY=   # Generate with: npx flowlib-cli secret
```
...
app.listen(3000, () => {
  console.log('Invect API at http://localhost:3000/flowlib');
});
```
...
This registers the `INVECT_CORE` provider (singleton), `InvectService` (injectable wrapper), and `InvectController` (REST endpoints under `/flowlib`).
...
When the `@flowlib/user-auth` plugin is included in `flowlib.config.ts`, `<Invect>` automatically wraps itself with an auth gate — no extra wrapper component needed. The OAuth callback route is handled internally at `<frontendPath>/oauth/callback`.
