...
description: Add user authentication with @flowlib/user-auth.
---
...
No extra wrapper component is needed. Pass the `auth()` plugin in `flowlib.config.ts` — `<Invect>` reads the config and automatically wraps itself with an auth gate:
