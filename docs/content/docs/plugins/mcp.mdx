...
description: Let AI assistants build, run, and debug flows with @flowlib/mcp.
---
...
```bash
export INVECT_URL=http://localhost:3000/flowlib
...
1. **Stdio (CLI)** — `npx flowlib-cli mcp` starts a standalone process that connects to Invect over HTTP. This is what MCP clients like Claude Desktop use.
