...
The configuration object is passed to `createInvectRouter()` (Express), `InvectModule.forRoot()` (NestJS), or `createInvectHandler()` (Next.js). It is also read directly by `<Invect config={...} />` on the frontend for fields like `apiPath`, `frontendPath`, `theme`, and `plugins`. Use `defineConfig()` from `@flowlib/core` for type inference.
...
export const invectConfig = defineConfig({
  ...
  frontendPath: '/flowlib',  // where <Invect> is mounted in the frontend router
...
The URL path where the Invect API is mounted (e.g. `'/api/flowlib'` or `'http://localhost:3000/flowlib'`). Read by `<Invect>` on the frontend to know where to send API requests.

## `frontendPath` (optional)

The URL path where `<Invect>` is mounted in the frontend router (e.g. `'/flowlib'`). Used for internal navigation and the OAuth callback route (`<frontendPath>/oauth/callback`).

## `theme` (optional)

`'light' | 'dark' | 'system'` — default `'system'`. Controls the editor color scheme.
