...
<Callout type="info">
  Next.js deployments on Vercel need one dedicated Invect cron route, configured in `vercel.json`,
  to drive scheduled work. Use a single Invect cron endpoint such as `/api/flowlib/cron`; it will
  execute due Invect cron triggers and other background maintenance for the app.
</Callout>
