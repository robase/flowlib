 ...
 * adding Flowlib. It's a simple SaaS admin backend with customers,
 * products, orders, and team members.
 *
 * When Flowlib is added, `npx flowlib-cli generate` merges Flowlib's
...
// ─── Flowlib type aliases (used by $type<>() for type-safe queries) ──
...
type FlowlibDefinitionRuntime = Record<string, unknown>;
type FlowRunStatus =
  | 'PENDING'
  | 'RUNNING'
  | 'COMPLETED'
...
// ─────────────────────────────────────────────────────────────
// Team Members (used by better-auth as the "flowlib_user" table)
// ─────────────────────────────────────────────────────────────
...
export const user = pgTable('flowlib_user', {
...
export const account = pgTable('flowlib_account', {
  id: text('id').primaryKey().notNull(),
...
export const session = pgTable('flowlib_session', {
  id: text('id').primaryKey().notNull(),
...
export const verification = pgTable('flowlib_verification', {
  id: text('id').primaryKey().notNull(),
...
export const flowAccess = pgTable('flowlib_flow_access', {
  id: uuid('id')
    .primaryKey()
    .notNull()
    .$default(() => randomUUID()),
...
export const chatMessages = pgTable('flowlib_chat_messages', {
  ...
  flowId: text('flow_id')
    .notNull()
    .references(() => flows.id, { onDelete: 'cascade' }),
  role: text('role').$type<'flowlib_user' | 'assistant' | 'system' | 'tool'>().notNull(),
...
export const rbac_scope_access = pgTable('flowlib_rbac_scope_access', {
  id: text('id').primaryKey().notNull(),
...
export const rbac_team_members = pgTable('flowlib_rbac_team_members', {
  id: text('id').primaryKey().notNull(),
  team_id: text('team_id')
    .notNull()
    .references(() => rbac_teams.id, { onDelete: 'cascade' }),
...
export const rbac_teams = pgTable('flowlib_rbac_teams', {
  id: text('id').primaryKey().notNull(),
  name: text('name').notNull(),
  description: text('description'),
